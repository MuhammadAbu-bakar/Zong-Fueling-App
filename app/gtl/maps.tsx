...
export default function GTLMapsScreen() {
  const { user } = useAuth();
  ...
  const fetchLocations = async () => {
    try {
      console.log('Fetching GTL locations for Mohammad Irfan...');
      // Fetch only locations for Mohammad Irfan
      const { data: locationData, error: locationError } = await supabase
        .from('Location')
        .select('NAME, "Lat/Long", "OPERATIONAL STATUS", Subregion, "CMPAK GTL"')
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>GTL Site {selectedSite?.NAME} Details</Text>
            <TouchableOpacity
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading GTL Site Locations...</Text>
      </View>
    );
  }
